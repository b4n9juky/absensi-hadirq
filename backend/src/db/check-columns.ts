...
async function main() {
  ...
  try {
    ...
    console.log('Query 1 succeeded! Rows returned:', (rows1 as any[]).length);
    ...
    console.log('Report Query succeeded! Rows returned:', (rows2 as any[]).length);
    if ((rows2 as any[]).length > 0) {
      console.log('First row data:', (rows2 as any[])[0]);
    }
  } catch (err: any) {
