  ...
  return (
    <>
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fadeIn">
        {[
          { label: 'Total Siswa', value: stats.totalStudents, icon: <Users className="w-5 h-5" />, color: 'from-blue-500 to-teal-500', iconBg: 'bg-blue-500/10 text-blue-400', textColor: 'text-white' },
          { label: 'Hadir Tepat Waktu', value: stats.presentCount, icon: <CheckCircle className="w-5 h-5" />, color: 'from-teal-500 to-emerald-500', iconBg: 'bg-teal-500/10 text-teal-400', textColor: 'text-teal-400' },
          { label: 'Siswa Terlambat', value: stats.lateCount, icon: <Clock className="w-5 h-5" />, color: 'from-amber-500 to-orange-500', iconBg: 'bg-amber-500/10 text-amber-400', textColor: 'text-amber-500' },
          { label: 'Belum Hadir / Absen', value: stats.absentCount, icon: <UserMinus className="w-5 h-5" />, color: 'from-red-500 to-rose-500', iconBg: 'bg-red-500/10 text-red-400', textColor: 'text-red-500' },
        ].map((card, i) => (
          <div key={i} className="bg-slate-900/40 border border-slate-900 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-800 transition-all">
      ...
      <section className="bg-slate-900/20 border border-slate-900/60 rounded-2xl p-6 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button onClick={handleResetFilters} className="px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-800 text-xs font-bold transition-all self-end">
            Atur Ulang Filter
          </button>
        </div>
                    ...
                    <tr key={row.id} className="hover:bg-slate-900/25 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1 text-slate-300">
                          ...
                          <span>Jarak: <span className="font-semibold text-slate-200">{row.distance?.toFixed(1) || '-'}m</span></span>
                        </div>
                        <div className="text-[9px] text-slate-500 mt-0.5">Akurasi GPS: {row.accuracy?.toFixed(1) || '-'}m</div>
                      </td>
      ...
      {previewPhoto && (
        <ModalShell title="Foto Bukti Selfie" onClose={() => { setPreviewPhoto(null); setPreviewDetails(null); }} maxWidth="md"
          footer={<button onClick={() => { setPreviewPhoto(null); setPreviewDetails(null); }} className="px-4 py-2 rounded-xl border border-slate-800 text-slate-400 font-bold hover:text-white text-xs">Tutup</button>}>
          <img src={previewPhoto} alt="Preview Selfie" className="w-full rounded-xl border border-slate-800" />
          {previewDetails && (
            <div className="bg-slate-950 rounded-xl p-4 space-y-2 text-xs border border-slate-800">
              <div className="flex justify-between"><span className="text-slate-400">Siswa</span><span className="text-white font-semibold">{previewDetails.student?.nis || '-'}</span></div>
              <div className="flex justify-between"><span className="text-slate-400">Status</span><span className={`font-bold ${previewDetails.status === 'LATE' ? 'text-amber-400' : 'text-emerald-400'}`}>{previewDetails.status}</span></div>
              <div className="flex justify-between"><span className="text-slate-400">Jam Masuk</span><span className="text-white">{formatTimeString(previewDetails.checkinTime)}</span></div>
              {previewDetails.checkoutTime && (
                <div className="flex justify-between"><span className="text-slate-400">Jam Pulang</span><span className="text-white">{formatTimeString(previewDetails.checkoutTime)}</span></div>
              )}
            </div>
          )}
        </ModalShell>
